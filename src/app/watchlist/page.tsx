...
export default async function WatchlistPage({
    ...
    searchParams: Promise<{ group?: string; sortBy?: string; sortOrder?: string; buyable?: string }>
}) {
    const { group, sortBy = 'marketCap', sortOrder = 'desc', buyable } = await searchParams
    ...
    // Native fields that Prisma can handle directly
    ...
    const rawStocks = await (db as any).stock.findMany({
        where: {
            watchlists: {
                some: { userId: 1 } // Only starred stocks
            },
            ...(group ? {
                groups: {
                    some: { name: group }
